...
class IdleControlsSystem extends DNASystem {
  ...
  onEntityUpdate(ts, entity) {
    if (inputManager.isActiveAction('LEFT') || inputManager.isActiveAction('RIGHT')) {
      ...
      dnaManager.addComponent(entity, new RunControlsComponent(4, 0));
      dnaManager.addComponent(entity, new RunComponent(4, 0));
    }
  }
  ...
  onActionOnce(actionId, entity) {
    if (actionId == 'UP') {
      ...
      dnaManager.addComponent(entity, new JumpComponent(-15, 10));
    }
  }
}
...
class ComboSystem extends DNASystem {
  ...
  onEntityUpdate(ts, entity) {
    ...
    for (const hit of hits) {
      const jas = new Gfx2SpriteJAS(); // create a new sprite for each to allow some hits at same time
...
class GravitySystem extends DNASystem {
  ...
  onEntityUpdate(ts, entity) {
    ...
    const footPos = position.y + gravity.collider;
    ...
    if (move.velocityY > 0) {
      if (footPos > floor.elevation) {
        position.y = floor.elevation - gravity.collider;
        move.velocityY = 0;
      ...
      for (let entity of dnaManager.findEntities('Platform')) {
        ...
        const delta = Math.abs(footPos - platform.elevation) - EPSILON;
        ...
        if (footPos > platform.elevation && delta <= move.velocityY && position.x >= minX && position.x <= maxX) {
          position.y = platform.elevation - gravity.collider;
          move.velocityY = 0;
          gravity.onFloor = true;
          return;
        }
...
async function CREATE_PLAYER1() {
  ...
  dnaManager.addComponent(player, new GravityComponent(1, 44));
