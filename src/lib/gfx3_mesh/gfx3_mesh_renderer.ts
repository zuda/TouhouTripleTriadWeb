...
class Gfx3MeshRenderer extends Gfx3RendererAbstract {
  ...
  pointLight0: vec16_buf;
  pointLight1: vec16_buf;
  dirLight: vec12_buf;
  fog: vec12_buf;
  ...
  constructor() {
    ...
    this.worldBuffer.addInput(4, UT.F12_SIZE, 'FOG');
    ...
    this.enablePointLight(0, 0, [0, 0, 0]);
    this.setPointLightAttenuation(0, 1, 0, 0);
    
    this.pointLight1 = new Float32Array(16);
    this.enablePointLight(1, 0, [0, 0, 0]);
    this.setPointLightAttenuation(1, 1, 0, 0);

    this.dirLight = new Float32Array(12);
    this.enableDirLight(0, [0, -1, 0]);

    this.fog = new Float32Array(12);
    this.enableFog(false, [0.5, 0.5, 0.5]);
  ...
  render(): void {
    ...
    this.worldBuffer.write(4, this.fog);
  ...
  enablePointLight(index: number, intensity: number, position: vec3): void {
    if (index == 0) {
  ...
  enableDirLight(intensity: number, direction: vec3): void {
  ...
  enableFog(enabled: boolean, color: vec3 = [0.5, 0.5, 0.5], near: number = 3.0, far: number = 15.0): void {
    ...
    this.fog[1] = near;
    this.fog[2] = far;
    this.fog[1 * 4 + 0] = color[0];
    this.fog[1 * 4 + 1] = color[1];
    this.fog[1 * 4 + 2] = color[2];
  }
}
